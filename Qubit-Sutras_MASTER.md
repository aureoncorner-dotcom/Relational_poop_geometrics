...
claim\_lane: "Modern quantum information expressed through poetic/machine-addressable teaching structure"
canonical\_warning: "This does not claim that ancient Sanskrit texts contained quantum theory. Sanskrit is used as a precision-poetic interface."
machine\_companions:

* "Qubit-Sutras\_INDEX.yaml"
* "Qubit-Sutras\_SUTRAS.jsonl"
* "appendices/APPENDIX\_MACHINE\_INGESTION.md"
...
* `Qubit-Sutras\\\\\\\_INDEX.yaml` — chapter-level concept map, prerequisites, tags, and cross-links.
* `Qubit-Sutras\\\\\\\_SUTRAS.jsonl` — one machine-addressable record per sutra.
* `appendices/APPENDIX\\\\\\\_MACHINE\\\\\\\_INGESTION.md` — schema notes, claim lanes, and ingestion guidance.
...
$$
\\rho = \\sum\_i p\_i |\\चित्\_i\\rangle\\langle\\चित्\_i|  
$$
...
|\\Psi\\rangle\_{\\text{initial}} =
\\left(  
\\alpha |\\शून्यम्\\rangle\_1  
+  
\\beta |\\एकम्\\rangle\_1  
\\right)  
\\otimes  
\\frac{  
|\\शून्यशून्यम्\\rangle\_{23}  
+  
|\\एकैकम्\\rangle\_{23}  
}{\\sqrt{2}}  
$$
...
\\frac{1}{2}  
\\left\[  
|\\शून्यशून्यम्\\rangle\_{12}  
\\left(  
\\alpha|\\शून्यम्\\rangle\_3 +  
\\beta|\\एकम्\\rangle\_3  
\\right)  
+  
|\\शून्यैकम्\\rangle\_{12}  
\\left(  
\\alpha|\\एकम्\\rangle\_3 +  
\\beta|\\शून्यम्\\rangle\_3  
\\right)  
\\right.  
$$
...
|Alice’s result $m\_1 m\_2$|Bob’s qubit before correction|Bob applies|Final state|
|-|-|-|-|
...
(H) = Hadamard gate  
(M) = measurement  
(m\_1,m\_2) = classical bits sent from Alice to Bob
...
$$
\\frac{1}{\\sqrt{2^t}}  
\\sum\_{k=0}^{2^t-1}|k\\rangle|\\psi\\rangle  
$$
...
$$
\\frac{1}{\\sqrt{2^t}}  
\\sum\_{k=0}^{2^t-1}|k\\rangle U^k|\\psi\\rangle  
$$
...
where each (P\_j) is a tensor product of Pauli operators:
...
E(\\theta) =
\\sum\_j c\_j  
\\langle \\psi(\\theta)|P\_j|\\psi(\\theta)\\rangle  
$$
...
$$
S = \\left| \\langle A\_1B\_1\\rangle + \\langle A\_1B\_2\\rangle + \\langle A\_2B\_1\\rangle - \\langle A\_2B\_2\\rangle \\right|
$$
...
$$
A - R\_1,\\quad R\_1 - R\_2,\\quad R\_2 - R\_3,\\quad R\_3 - B  
$$
...
A Bell measurement on (M\_1M\_2) projects (A) and (B) into entanglement, up to correction.
